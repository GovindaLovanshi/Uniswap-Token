...
const Home = ({setActiveComponent,GET_POOL_ADDRESS}) => {
  const[selectrNetwork,setSelectNetwork] = useState({});
  ...
  useEffect(()=>{
    ...
    selectrNetwork(network);
  },[]);

  // CALLING FUn
  const CAAL_POOL_DETAIL = async(inputAddress) =>{
    const {rpcUrl} = selectrNetwork;
    ...
    const poolDetails = await GET_POOL_ADDRESS(inputAddress,selectrNetwork);
    setPoolDetails(poolDetails);
  };
  return (
    <section className="relative overflow-hidden pt-[72px] bg-default-950/40 backdrop-blur-3xl">
      <div className="absolute h-14 w-14 bg-primary/10 top-2/3
      start-80 -z-1 rounded-2xl rounded-tl-none rounded-br-none
             ...
             <div className="py-14 text-center relative">
               <div className="flex justify-center">
                   ...
                   <div className="w-full flex items-center justify-between mt-7">
                     <input type="text"
                      className="w-full p-4 border-0 focus:outline-none
                       focus:ring-0 text-sm text-white placeholder:text-white bg-transparent" 
                     placeholder="Enter pool Address" autoComplete="off"
                     onChange={(e)=> setPoolAddress(e.target.value)}/>

                     <button className="py-2 px-6 me-2 border-0 text-white font-semibold text-sm 
                      rounded-md backdrop-blur-2xl bg-primary hover:bg-primary-hover hover:text-white transition-all duration-500">
                        <div onClick={() => CAAL_POOL_DETAIL(poolAddress)} className="flext items-center justify-center gap-1">
                          <span>Submit</span>
                        </div>
                      </button>
                   </div>
                 ...
                 <div className="flex flex-wrap items-center justify-center gap-6 mt-10">
                   <a onClick={()=> setActiveComponent("Liquidity History")}
                   ...
                   <a onClick={()=> setActiveComponent("Liquidity")}
